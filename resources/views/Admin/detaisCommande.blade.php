...
@section('cmdStyle')
    <style>
        ...
        .img .img-content .span #description_D {
            font-weight: 400;
            opacity: 0;
        ...
        .img .img-content:hover .span #description_D{
            opacity: 1;
            transform: scale(1);
            padding-bottom: 20px;
        }
...
@section('navcontent')
    <div class="container">
        <div class="dash-content">
            <div class="overview">
                <div class="header">
                    <a href="{{ route('home') }}"><i class="uil uil-arrow-left"></i></a>
                </div>
                        ...
                        <tbody>
                            @foreach ($cmds as $cmd)
                                <tr class="tr">
                                    ...
                                    <td class="showDetais_D" data-bs-toggle="modal" data-bs-target="#exampleModal"><img
                                            src="{{ asset($cmd->img) }}" alt="" width="70px"></td>
                                    <td class="showDetais_D" data-bs-toggle="modal" data-bs-target="#exampleModal">
                                        {{ $cmd->nom }}</td>
                                    <td class="showDetais_D" data-bs-toggle="modal" data-bs-target="#exampleModal">
                                        {{ $cmd->prenom }}</td>
                                    <td class="showDetais_D" data-bs-toggle="modal" data-bs-target="#exampleModal">
                                    ...
                                    @endif
                                    <td id="showDetais_D" data-bs-toggle="modal" data-bs-target="#exampleModal">
                                ...
                                <div class="row d-flex justify-content-between">
                                    <div class="col-6">
                                        ...
                                        <div class="info total d-flex align-items-center justify-content-between">
                                            <h6 id="qty_D" class="me-3 mt-2"></h6>
                                            <div>
                                                <h6 class="d-inline me-1">Totale :</h6>
                                                <span id="total_D" class="text-light"></span>
                                            </div>
                                        </div>
                                    ...
                                    <div class="col-6 img">
                                        <div class="img-content">
                                            <img src="" alt="" id="img_D" width="100%">
                                            <div class="span d-flex flex-column">
                                                <div class="span-info d-flex align-items-center justify-content-between">
                                                    <span id="produit_D" class="text-light"></span>
                                                    <span id="prix_D" class="text-light px-2 fs-5"></span>
                                                </div>
                                                <div id="description_D" class="text-light fs-6 pt-4"></div>
                                            </div>
                                        </div>
...
<script>
    $(document).ready(function() {
        $('.showDetais_D').on('click', function() {
            $tr = $(this).closest('tr')
            var data = $tr.children('td').map(function() {
                return $(this).text();
            }).get();
            console.log(data)
            $('#nom_D').text(data[3] + ' ' + data[4])
            // $('#prenom').text(data[4])
            $('#tel_D').text(data[5])
            $('#ville_D').text(data[6])
            $('#qty_D').text('Quantité :' + data[7])
            $('#date_D').text(data[8])
            $('#time_D').text(data[9])
            $('#total_D').text(data[11])
            $('#address_D').text(data[13])
            $('#detais_D').text(data[12])
            $('#produit_D').text(data[14])
            $('#prix_D').text(data[15] + ' DH')
            $('#description_D').text(data[16])
            $('#img_D').attr('src', data[1])
        })
    })
</script>
@endsection
